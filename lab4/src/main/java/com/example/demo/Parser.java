...
public class Parser {
    public void writeCoordinates(File file, ResultRepository resultRepository) {
        List<Model> modelList = this.parseCoordinates(file);

        for (Model i : modelList) {
            String result = Convert.latLonToString(new Model(i.getLat(), i.getLon()), 90, 180);
            resultRepository.save(new ResultModel(i.getLat(), i.getLon(), result));
        }
    }

    private List<Model> parseCoordinates(File inputFile) {
